...
const ProtectEggGame = () => {
  ...
  let ctx;
  /** @type {import('./components/GameObject').default[]} */
  const gameObjects = [];
  ...
  const start = () => {
    ...
    window.addEventListener(eventName.tickAdd, () => {
      ...
      if (gameObjects.some(findEnemy)) {
        ...
        const currSpeed = reuseEnemy.getData('speed');
        const updateSpeed = Math.random() > 0.65 ? currSpeed : currSpeed * (1 + Math.random() * 0.25);
        reuseEnemy.setData('speed', Math.min(updateSpeed, 1));
      ...
      enemy.setData('speed', 0.25).setSprite(imageManager.get('piko-sus'));
    ...
    window.addEventListener("keydown", e => {
      if (gameState === GAME_STATE.PLAY && e.code === 'Space') {
        if (!justPressSpace) {
          playerShoot();
          justPressSpace = true;
        }
